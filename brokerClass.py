...
class DhanAPICleint(metaclass=SingletonMeta):
    ...
    def login(self):
        accessToken = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiJ9.eyJpc3MiOiJkaGFuIiwicGFydG5lcklkIjoiIiwiZXhwIjoxNzc1NDkzMjE2LCJpYXQiOjE3NzU0MDY4MTYsInRva2VuQ29uc3VtZXJUeXBlIjoiU0VMRiIsIndlYmhvb2tVcmwiOiIiLCJkaGFuQ2xpZW50SWQiOiIxMTAwMjY1NzE3In0.DlENw3mbrMua6CfofD7aoYnrBL-210BhcY83NCM_LrEqt-NRgLU9ZA0YCD4J3IzxCZ7qVejRCMoCVEJXBGA07w'
        self.dhan_context = DhanContext(self.clientId,accessToken)
        self.dhan = dhanhq(self.dhan_context)
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if  response and response['type'] in ['Ticker Data' ,'Full Data']:
                        self.liveFeed[str(response['security_id'])] = {'ltp':float(response['LTP']), 'ltt':  datetime.strptime(f"{datetime.now(self.timeZone).date()} {response['LTT']}", "%Y-%m-%d %H:%M:%S").replace(tzinfo=self.timeZone)}
                else:
    ...
    def get_historical_daily_candles(self, security_id, exchange_segment, instrument_type,
        ...
        return candledf
    def get_ticker_response(self,instrument:list):
        try:
            res=self.dhan.ticker_data(
            securities = {'NSE_FNO':instrument }
            )
            return res   
        except Exception:
            return "error in getting ticker response "